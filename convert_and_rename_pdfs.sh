...
for ((PAGE=0; PAGE<TOTAL_PAGES; PAGE++)); do
    ...
    NEW_IMAGE_INDEX=$(printf "%03d" $((PAGE+158)))
    TEMP_FILE="/tmp/temp_page_qldzj.tiff"
    OUTPUT_FILE="$OUTPUT_DIR/$NEW_IMAGE_INDEX.tiff"
...
# 重命名现有文件以从158开始计数
for FILE in "$OUTPUT_DIR"/{001..157}.tiff; do
    if [ -e "$FILE" ]; then
        BASENAME=$(basename "$FILE")
        NEW_NAME=$(printf "%03d.tiff" $((10#${BASENAME:0:3} + 157)))
        mv "$FILE" "$OUTPUT_DIR/$NEW_NAME"
        echo "Renamed $BASENAME to $NEW_NAME"
    fi
done

echo "All pages of $PDF_FILE have been successfully converted and saved in $OUTPUT_DIR with renamed files starting from 158.tiff."
