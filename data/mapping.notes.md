...
`data/mapping.json` 的定位统一用 `new URL('./data/mapping.json', import.meta.url)`，根目录页面与 `/erratum/` 子目录页面均可访问。

## 生成方式

由 `back/tools/gen_unified_mapping.mjs` 生成：
- 输入：`back/tools/legacy_src/mapping.json`（旧每卷数组结构）+ `back/tools/legacy_src/catalog_imgbb.js`（原 C 图映射，一次性迁移后存档于此）
- 输出：上述新结构 `data/mapping.json`
